...
"use client";
var LayoutGroupContext = import_react4.createContext({});

// node_modules/framer-motion/dist/es/utils/use-constant.mjs
var import_react5 = __toESM(require_react(), 1);
"use client";
function useConstant(init) {
  const ref = import_react5.useRef(null);
  if (ref.current === null) {
    ref.current = init();
  }
  return ref.current;
...
"use client";
var PresenceContext = /* @__PURE__ */ import_react7.createContext(null);
...
"use client";
var MotionConfigContext = import_react8.createContext({
...
function PopChild({ children, isPresent, anchorX, anchorY, root, pop }) {
  ...
  const size = import_react9.useRef({
    width: 0,
    height: 0,
    top: 0,
    left: 0,
  ...
  const { nonce } = import_react9.useContext(MotionConfigContext);
  const childRef = children.props?.ref ?? children?.ref;
  const composedRef = useComposedRefs(ref, childRef);
  import_react9.useInsertionEffect(() => {
...
var PresenceChild = ({ children, initial, isPresent, onExitComplete, custom, presenceAffectsLayout, mode, anchorX, anchorY, root }) => {
  ...
  let context = import_react10.useMemo(() => {
...
function usePresence(subscribe = true) {
  const context = import_react11.useContext(PresenceContext);
  if (context === null)
    return [true, null];
  ...
  const safeToRemove = import_react11.useCallback(() => subscribe && onExitComplete && onExitComplete(id2), [id2, onExitComplete, subscribe]);
...
function onlyElements(children) {
  ...
  import_react12.Children.forEach(children, (child) => {
    if (import_react12.isValidElement(child))
      filtered.push(child);
  });
  return filtered;
}
...
var AnimatePresence = ({ children, custom, initial = true, onExitComplete, presenceAffectsLayout = true, mode = "sync", propagate = false, anchorX = "left", anchorY = "top", root }) => {
  ...
  const presentChildren = import_react13.useMemo(() => onlyElements(children), [children]);
  const presentKeys = propagate && !isParentPresent ? [] : presentChildren.map(getChildKey);
  const isInitialRender = import_react13.useRef(true);
  const pendingPresentChildren = import_react13.useRef(presentChildren);
  const exitComplete = useConstant(() => new Map);
  const exitingComponents = import_react13.useRef(new Set);
  const [diffedChildren, setDiffedChildren] = import_react13.useState(presentChildren);
  const [renderedChildren, setRenderedChildren] = import_react13.useState(presentChildren);
  ...
  const { forceRender } = import_react13.useContext(LayoutGroupContext);
...
var createDomVisualElement = (Component2, options) => {
  ...
  return isSVG ? new SVGVisualElement(options) : new HTMLVisualElement(options, {
    allowProjection: Component2 !== import_react14.Fragment
  });
};
...
"use client";
var LazyContext = import_react15.createContext({ strict: false });

// node_modules/framer-motion/dist/es/context/MotionContext/index.mjs
var import_react16 = __toESM(require_react(), 1);
"use client";
var MotionContext = /* @__PURE__ */ import_react16.createContext({});
...
function useCreateMotionContext(props) {
  const { initial, animate } = getCurrentTreeVariants(props, import_react17.useContext(MotionContext));
  return import_react17.useMemo(() => ({ initial, animate }), [variantLabelsAsDependency(initial), variantLabelsAsDependency(animate)]);
}
...
function useSVGProps(props, visualState, _isStatic, Component2) {
  const visualProps = import_react19.useMemo(() => {
...
function useRender(Component2, props, ref, { latestValues }, isStatic, forwardMotionProps = false, isSVG) {
  ...
  const renderedChildren = import_react20.useMemo(() => isMotionValue(children) ? children.get() : children, [children]);
  return import_react20.createElement(Component2, {
    ...elementProps,
...
var makeUseVisualState = (config) => (props, isStatic) => {
  const context = import_react21.useContext(MotionContext);
  const presenceContext = import_react21.useContext(PresenceContext);
...
function useMotionRef(visualState, visualElement, externalRef) {
  const externalRefContainer = import_react22.useRef(externalRef);
  import_react22.useInsertionEffect(() => {
    externalRefContainer.current = externalRef;
  });
  const refCleanup = import_react22.useRef(null);
  return import_react22.useCallback((instance) => {
    if (instance) {
...
"use client";
var SwitchLayoutGroupContext = import_react23.createContext({});
...
function useVisualElement(Component2, visualState, props, createVisualElement, ProjectionNodeConstructor, isSVG) {
  const { visualElement: parent } = import_react24.useContext(MotionContext);
  const lazyContext = import_react24.useContext(LazyContext);
  const presenceContext = import_react24.useContext(PresenceContext);
  const motionConfig = import_react24.useContext(MotionConfigContext);
  const reducedMotionConfig = motionConfig.reducedMotion;
  const skipAnimations = motionConfig.skipAnimations;
  const visualElementRef = import_react24.useRef(null);
  const hasMountedOnce = import_react24.useRef(false);
  ...
  const initialLayoutGroupConfig = import_react24.useContext(SwitchLayoutGroupContext);
  ...
  const isMounted = import_react24.useRef(false);
  import_react24.useInsertionEffect(() => {
    if (visualElement && isMounted.current) {
      visualElement.update(props, presenceContext);
    }
  });
  const optimisedAppearId = props[optimizedAppearDataAttribute];
  const wantsHandoff = import_react24.useRef(Boolean(optimisedAppearId) && typeof window !== "undefined" && !window.MotionHandoffIsComplete?.(optimisedAppearId) && window.MotionHasOptimisedAnimation?.(optimisedAppearId));
  useIsomorphicLayoutEffect(() => {
...
function createMotionComponent(Component2, { forwardMotionProps = false, type } = {}, preloadedFeatures, createVisualElement) {
  ...
  function MotionDOMComponent(props, externalRef) {
    ...
    const configAndProps = {
      ...import_react25.useContext(MotionConfigContext),
      ...props,
  ...
  const ForwardRefMotionComponent = import_react25.forwardRef(MotionDOMComponent);
  ForwardRefMotionComponent[motionComponentSymbol] = Component2;
  return ForwardRefMotionComponent;
}
function useLayoutId({ layoutId }) {
  const layoutGroupId = import_react25.useContext(LayoutGroupContext).id;
  return layoutGroupId && layoutId !== undefined ? layoutGroupId + "-" + layoutId : layoutId;
}
function useStrictMode(configAndProps, preloadedFeatures) {
  const isStrict = import_react25.useContext(LazyContext).strict;
...
class MeasureLayoutWithContext extends import_react26.Component {
  componentDidMount() {
...
function MeasureLayout(props) {
  ...
  const layoutGroup = import_react26.useContext(LayoutGroupContext);
  return import_jsx_runtime5.jsx(MeasureLayoutWithContext, { ...props, layoutGroup, switchLayoutGroup: import_react26.useContext(SwitchLayoutGroupContext), isPresent, safeToRemove });
}
...
var FlipWords = ({
  ...
  const [currentWord, setCurrentWord] = import_react27.useState(words[0]);
  const [isAnimating, setIsAnimating] = import_react27.useState(false);
  const startAnimation = import_react27.useCallback(() => {
  ...
  return /* @__PURE__ */ jsx_dev_runtime5.jsxDEV(AnimatePresence, {
    children: /* @__PURE__ */ jsx_dev_runtime5.jsxDEV(motion.span, {
...
//# debugId=CD3ECA6B07052EC164756E2164756E21
//# sourceMappingURL=chunk-9p4p2xae.js.map
